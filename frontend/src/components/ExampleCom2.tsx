...
// Define the interface for a single requirement
...
const AdmissionRequirements2: React.FC = () => {
  ...
  const renderRequirement = useCallback(
    (req: Requirement, index: number, parentIndexes: number[] = []) => {
      ...
      const prefix =
        numberingStyle === 'none' ? '' : numberingStyles[numberingStyle](index, req.level)
          ...
          {[1, 2, 3].map((level) => (
            <div key={level} className="flex items-center">
              ...
              <SelectMenu
                ...
                onChange={(newStyle) => handleStyleChange(level - 1, newStyle)}
...
export default AdmissionRequirements2
