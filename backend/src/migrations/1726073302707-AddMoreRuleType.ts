...
export class AddMoreRuleType1726073302707 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`preset_requirements\` CHANGE \`style\` \`style\` varchar(255) NOT NULL DEFAULT 'aa.ts'`);
        ...
        await queryRunner.query(`ALTER TABLE \`requirements\` ADD \`style\` enum ('numeric', 'alphabetic', 'roman', 'none') NOT NULL DEFAULT 'numeric'`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`requirements\` DROP COLUMN \`style\``);
        await queryRunner.query(`ALTER TABLE \`requirements\` ADD \`style\` varchar(255) NOT NULL DEFAULT 'aa'`);
        ...
        await queryRunner.query(`ALTER TABLE \`courses\` CHANGE \`type\` \`type\` enum ('Graduate Certificate', 'Graduate Diploma', 'Master''''s (Coursework)', 'Master''''s Extended', 'Master''''s Research', 'Doctoral Degree', 'Professional Doctorate/Master''''s Coursework') NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`preset_requirements\` CHANGE \`style\` \`style\` varchar(255) NOT NULL DEFAULT 'aa'`);
    }
